...
export async function ensureSocialSchema() {
  ...
  `);
  await db.execute(sql`ALTER TABLE post_reposts ADD COLUMN IF NOT EXISTS id uuid DEFAULT gen_random_uuid()`);
  await db.execute(sql`ALTER TABLE post_reposts ALTER COLUMN id SET NOT NULL`);
  await db.execute(sql`ALTER TABLE post_reposts DROP CONSTRAINT IF EXISTS post_reposts_pkey`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS post_reposts_post_idx ON post_reposts(post_id,created_at DESC)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS post_reposts_user_idx ON post_reposts(user_id,created_at DESC)`);

  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS agent_verification_requests(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      organization_name varchar(180) NOT NULL, organization_type varchar(80) NOT NULL, website text NOT NULL, domain varchar(255) NOT NULL,
      country varchar(120) NOT NULL, description text NOT NULL, dns_record_name varchar(320) NOT NULL, dns_record_value varchar(320) NOT NULL UNIQUE,
      dns_verified boolean NOT NULL DEFAULT false, status varchar(24) NOT NULL DEFAULT 'pending_dns', verification_note text, submitted_at timestamptz,
      reviewed_at timestamptz, reviewer_id uuid REFERENCES users(id), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
    )
  `);
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS project_collaborators(
      ...
      user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      status varchar(20) NOT NULL DEFAULT 'pending', role varchar(20) NOT NULL DEFAULT 'editor', created_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(project_id,user_id)
    )
  `);
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS agent_affiliation_requests(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(), agent_id uuid NOT NULL REFERENCES agents(id) ON DELETE CASCADE, user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      role varchar(160) NOT NULL, status varchar(20) NOT NULL DEFAULT 'pending', created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), UNIQUE(agent_id,user_id)
    )
  `);
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS agent_affiliations(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(), agent_id uuid NOT NULL REFERENCES agents(id) ON DELETE CASCADE, user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      role varchar(160) NOT NULL, verified_at timestamptz NOT NULL DEFAULT now(), UNIQUE(agent_id,user_id)
    )
  `);
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS agent_role_requests(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(), agent_id uuid NOT NULL REFERENCES agents(id) ON DELETE CASCADE, user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      "current_role" varchar(160), requested_role varchar(160) NOT NULL, status varchar(20) NOT NULL DEFAULT 'pending', created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
    )
  `);
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS agent_affiliation_history(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(), agent_id uuid NOT NULL REFERENCES agents(id) ON DELETE CASCADE, user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      role varchar(160) NOT NULL, event_type varchar(30) NOT NULL DEFAULT 'role_change', created_at timestamptz NOT NULL DEFAULT now()
    )
  `);

  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS hashtags(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tag varchar(64) NOT NULL UNIQUE,created_at timestamptz NOT NULL DEFAULT now())
  `);
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS post_hashtags(
      post_id uuid NOT NULL REFERENCES posts(id) ON DELETE CASCADE, hashtag_id uuid NOT NULL REFERENCES hashtags(id) ON DELETE CASCADE,

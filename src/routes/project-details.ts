...
function rows<T extends Row>(result: unknown): T[] { const value=result as T[]|{rows?:T[]}; return Array.isArray(value)?value:value.rows??[]; }

projectDetailsRouter.get("/projects/:slug", async (req, res) => {
  if (!db) return res.status(404).json({ error: "Project not found" });
  try {
    const result=await db.execute(sql`SELECT p.id,p.name,p.slug,p.description,p.stage,p.github_url,p.created_at,
    ...
    const postsResult=await db.execute(sql`SELECT p.id,p.body,p.created_at FROM posts p WHERE p.project_id=${row.id} ORDER BY p.created_at ASC LIMIT 100`);
    const contributorsResult=await db.execute(sql`SELECT pc.user_id,pc.status,pc.created_at,u.name,u.username,u.avatar_url,u.account_type FROM project_collaborators pc JOIN users u ON u.id=pc.user_id WHERE pc.project_id=${row.id} AND pc.status='accepted' ORDER BY pc.created_at ASC`);
    return res.json({data:{id:row.id,name:row.name,slug:row.slug,description:row.description,stage:row.stage,githubUrl:row.github_url,createdAt:row.created_at,owner:{id:row.owner_id,name:row.owner_name,username:row.owner_username,avatarUrl:row.owner_avatar},agent:row.agent_id?{id:row.agent_id,name:row.agent_name,slug:row.agent_slug,domain:row.agent_domain,website:row.agent_website,verified:row.agent_verified}:null,contributors:rows(contributorsResult),posts:rows(postsResult)}});
  }catch(error){console.error("[ProjectDetails] Failed to load project:",error);return res.status(500).json({error:"Unable to load project details"});}
});
...
projectDetailsRouter.patch("/projects/:slug", requireAuth, async (req,res)=>{
  ...
  try{
    ...
    const result=await db.execute(sql`SELECT id,owner_id FROM projects WHERE lower(slug)=lower(${req.params.slug}) LIMIT 1`);
    const project=rows(result)[0];

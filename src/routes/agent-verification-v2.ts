...
async function reviewer(req: Request & { verificationReviewer?: string }, res: Response, next: NextFunction) {
...
agentVerificationRouter.post("/apply", requireAuth, async (req, res) => {
  ...
  let domain: string;
  try { domain = normalizeDomain(parsed.data.domain); } catch { return res.status(400).json({ error: "Enter a valid organization domain, for example example.com." }); }

  const existing = await rows(sql`SELECT * FROM agent_verification_requests WHERE user_id=${req.auth!.subjectId} AND status IN ('pending_dns','pending_review') ORDER BY created_at DESC LIMIT 1`);
...
agentVerificationRouter.get("/me", requireAuth, async (req, res) => {
  ...
  const request = await rows(sql`SELECT * FROM agent_verification_requests WHERE user_id=${req.auth!.subjectId} ORDER BY created_at DESC LIMIT 1`);
...
agentVerificationRouter.post("/:id/verify-dns", requireAuth, async (req, res) => {
  if (!db) return res.status(503).json({ error: "Database unavailable" });
  const requestRows = await rows(sql`SELECT * FROM agent_verification_requests WHERE id=${req.params.id} AND user_id=${req.auth!.subjectId} LIMIT 1`);
  ...
  try {
    ...
    if (!values.includes(request.dns_record_value)) return res.status(422).json({ error: "The DNS TXT record was not found yet. DNS changes can take some time to propagate." });
  } catch {
    return res.status(422).json({ error: "We could not find the DNS TXT record yet. Check the record name and value, then try again." });
  }

  const updated = await rows(sql`UPDATE agent_verification_requests SET dns_verified=true,status='pending_review',submitted_at=now(),updated_at=now() WHERE id=${request.id} RETURNING *`);
...
agentVerificationRouter.post("/:id/review", requireAuth, reviewer, async (req: Request & { verificationReviewer?: string }, res) => {
  if (!db) return res.status(503).json({ error: "Database unavailable" });
  ...
  const requestRows = await rows(sql`SELECT r.*, u.name AS applicant_name, u.username AS applicant_username FROM agent_verification_requests r JOIN users u ON u.id=r.user_id WHERE r.id=${req.params.id} LIMIT 1`);
  const request = requestRows[0];
  if (!request) return res.status(404).json({ error: "Verification request not found." });
  ...
  if (parsed.data.decision === "reject") {
    await db.execute(sql`UPDATE agent_verification_requests SET status='rejected',verification_note=${parsed.data.note || "The verification team could not approve this organization."},reviewed_at=now(),reviewer_id=${req.verificationReviewer ?? null},updated_at=now() WHERE id=${request.id}`);
  ...
  await db.execute(sql`
    ...
    ON CONFLICT (id) DO UPDATE SET
      name=EXCLUDED.name, slug=EXCLUDED.slug, type=EXCLUDED.type, verified=true,
      verification_status='approved', verification_note=EXCLUDED.verification_note,
      reviewed_at=now(), domain=EXCLUDED.domain, website=EXCLUDED.website
  `);
  await db.execute(sql`UPDATE users SET account_type='agent',onboarding_completed=true WHERE id=${request.user_id}`);
  await db.execute(sql`UPDATE agent_verification_requests SET status='approved',verification_note=${parsed.data.note || null},reviewed_at=now(),reviewer_id=${req.verificationReviewer ?? null},updated_at=now() WHERE id=${request.id}`);

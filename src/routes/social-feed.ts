...
async function postRows(mode: "for-you" | "network", viewerId?: string) {
  if (!db) return [] as Row[];
  const network = mode === "network" && viewerId ? sql`AND (p.author_id = ${viewerId} OR EXISTS (SELECT 1 FROM follows nf WHERE nf.follower_id = ${viewerId} AND nf.following_id = p.author_id))` : sql``;
  const viewer = viewerId ? sql`LEFT JOIN follows vf ON vf.follower_id = ${viewerId} AND vf.following_id = p.author_id` : sql``;
  const result = await db.execute(sql`
    ...
    LIMIT 160
  `);
  return (result as unknown as Row[]).map((row) => ({ ...row, score: feedScore(row) })).sort((a, b) => b.score - a.score);
}
...
socialFeedRouter.get("/feed", async (req, res) => {
  ...
  const rows = await postRows(mode, req.auth?.subjectId);
  ...
  res.json({ data: rows.slice(0, 50).map((r) => serializePost({ ...r, ...(stateMap.get(r.id) ?? {}) })), algorithm: "nerdding-v2-transparent-interest-score", mode });
});

async function loadComments(postId: string) {
  if (!db) return [];
  const result = await db.execute(sql`SELECT c.id, c.post_id, c.parent_id, c.body, c.created_at, u.id AS author_id, u.name, u.username, u.avatar_url
    FROM post_comments c JOIN users u ON u.id=c.author_id WHERE c.post_id=${postId} ORDER BY c.created_at ASC`);
  const rows = result as unknown as Row[];
  const byParent = new Map<string | null, any[]>();
  for (const row of rows) {
    const item = { id: row.id, postId: row.post_id, parentId: row.parent_id, body: row.body, createdAt: row.created_at, author: { id: row.author_id, name: row.name, username: row.username, avatarUrl: row.avatar_url }, replies: [] as any[] };
    const key = row.parent_id ?? null;
    const list = byParent.get(key) ?? []; list.push(item); byParent.set(key, list);
  }
  const attach = (items: any[]) => items.map((item) => ({ ...item, replies: attach(byParent.get(item.id) ?? []) }));
...
socialFeedRouter.get("/posts/:postId", async (req, res) => {
  ...
  const [post] = (await db.execute(sql`SELECT p.*, u.name, u.username, u.avatar_url, u.account_type, u.bio, u.location, u.trust_score,
      ...
      p.project_id, p.quote_post_id FROM posts p JOIN users u ON u.id=p.author_id LEFT JOIN projects pr ON pr.id=p.project_id WHERE p.id=${req.params.postId}`) as unknown as Row[]);
  if (!post) return res.status(404).json({ error: "Post not found" });
  res.json({ data: { ...serializePost(post), commentsTree: await loadComments(String(req.params.postId)) } });
});
...
socialFeedRouter.post("/posts/:postId/comments", requireAuth, async (req, res) => {
  ...
  const [row] = (await db.execute(sql`INSERT INTO post_comments (id, post_id, author_id, parent_id, body) VALUES (gen_random_uuid(), ${req.params.postId}, ${req.auth!.subjectId}, ${parsed.data.parentId ?? null}, ${parsed.data.body}) RETURNING id, post_id, parent_id, body, created_at`) as unknown as Row[]);
  ...
  res.status(201).json({ data: { ...row, author } });
});
...
socialFeedRouter.post("/posts/:postId/quote", requireAuth, async (req, res) => {
  ...
  const [row] = (await db.execute(sql`INSERT INTO posts (id, author_id, body, quote_post_id) VALUES (gen_random_uuid(), ${req.auth!.subjectId}, ${parsed.data.body}, ${req.params.postId}) RETURNING id, body, created_at`) as unknown as Row[]);
  res.status(201).json({ data: row });
});

socialFeedRouter.get("/users/:username/profile", async (req, res) => {
  if (!db) return res.status(404).json({ error: "Profile not found" });
  const [user] = (await db.execute(sql`SELECT id,name,username,email,avatar_url,bio,location,account_type,interests,trust_score,created_at FROM users WHERE lower(username)=lower(${req.params.username}) LIMIT 1`) as unknown as Row[]);
  ...
  const projects = await db.execute(sql`SELECT id,name,slug,description,stage,github_url,created_at FROM projects WHERE owner_id=${user.id} ORDER BY created_at DESC LIMIT 24`);
  const followers = await db.execute(sql`SELECT u.id,u.name,u.username,u.avatar_url FROM follows f JOIN users u ON u.id=f.follower_id WHERE f.following_id=${user.id} ORDER BY f.created_at DESC LIMIT 8`);
  const following = await db.execute(sql`SELECT u.id,u.name,u.username,u.avatar_url FROM follows f JOIN users u ON u.id=f.following_id WHERE f.follower_id=${user.id} ORDER BY f.created_at DESC LIMIT 8`);
  const viewerFollowing = req.auth ? await db.execute(sql`SELECT 1 FROM follows WHERE follower_id=${req.auth.subjectId} AND following_id=${user.id} LIMIT 1`) : [];
  const mutuals = req.auth ? await db.execute(sql`SELECT u.id,u.name,u.username,u.avatar_url FROM follows a JOIN follows b ON b.following_id=a.following_id JOIN users u ON u.id=a.following_id WHERE a.follower_id=${req.auth.subjectId} AND b.follower_id=${user.id} LIMIT 6`) : [];
  return res.json({ data: { user: { ...user, avatarUrl: user.avatar_url }, stats: counts, isFollowing: Array.isArray(viewerFollowing) ? viewerFollowing.length > 0 : false, projects, followers, following, mutualFollowers: mutuals } });
});

socialFeedRouter.get("/users/:userId/followers", requireAuth, async (req, res) => {
  if (!db) return res.json({ data: [] });
  const rows = await db.execute(sql`SELECT u.id,u.name,u.username,u.avatar_url,u.account_type FROM follows f JOIN users u ON u.id=f.follower_id WHERE f.following_id=${req.params.userId} ORDER BY f.created_at DESC`);
  res.json({ data: rows });
});

socialFeedRouter.get("/users/:userId/following", requireAuth, async (req, res) => {
  if (!db) return res.json({ data: [] });
  const rows = await db.execute(sql`SELECT u.id,u.name,u.username,u.avatar_url,u.account_type FROM follows f JOIN users u ON u.id=f.following_id WHERE f.follower_id=${req.params.userId} ORDER BY f.created_at DESC`);
  res.json({ data: rows });
});

socialFeedRouter.get("/explore/live", async (_req, res) => {
  const rows = await postRows("for-you");
  ...
  for (const row of rows) {
    ...
    current.posts += 1; current.engagement += Number(row.likes) + Number(row.comments) * 2 + Number(row.reposts) * 2;
    topics.set(topic, current);
  }
  res.json({ data: { stories: rows.slice(0, 30).map(serializePost), topics: [...topics.values()].sort((a,b) => b.engagement-a.engagement).slice(0, 12) }, algorithm: "velocity-recency-quality-v2" });
});
...
socialFeedRouter.get("/charts/live", async (_req, res) => {
  ...
  const builders = await db.execute(sql`SELECT u.id,u.name,u.username,u.avatar_url,u.account_type,u.trust_score,
    ...
    FROM users u LEFT JOIN posts p ON p.author_id=u.id LEFT JOIN post_likes pl ON pl.post_id=p.id LEFT JOIN post_comments pc ON pc.post_id=p.id LEFT JOIN follows f ON f.following_id=u.id
    GROUP BY u.id ORDER BY (COUNT(DISTINCT p.id)*3 + COUNT(DISTINCT pl.post_id)*2 + COUNT(DISTINCT pc.id)*3 + COUNT(DISTINCT f.follower_id)*0.2 + u.trust_score) DESC LIMIT 20`);
  const projects = await db.execute(sql`SELECT p.id,p.name,p.slug,p.description,p.stage,p.github_url, COUNT(DISTINCT ps.user_id)::int saves, COUNT(DISTINCT po.user_id)::int reposts FROM projects p LEFT JOIN posts x ON x.project_id=p.id LEFT JOIN post_saves ps ON ps.post_id=x.id LEFT JOIN post_reposts po ON po.post_id=x.id GROUP BY p.id ORDER BY saves DESC, reposts DESC LIMIT 12`);
  res.json({ data: { risingBuilders: builders, topProjects: projects, trendingStartups: [], activeCommunities: [] }, algorithm: "proof-collaboration-consistency-v2" });
});
...
socialFeedRouter.get("/projects/:slug/github-commits", async (req, res) => {
  ...
  try {
    const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/commits?per_page=12`, { headers: { Accept: "application/vnd.github+json", "User-Agent": "Nerddings" } });
    if (!response.ok) return res.json({ data: [] });
    const commits = await response.json() as any[];
    res.json({ data: commits.map((c) => ({ sha: c.sha, message: c.commit?.message?.split("\\n")[0] ?? "Commit", author: c.author?.login ?? c.commit?.author?.name ?? "Unknown", avatarUrl: c.author?.avatar_url ?? null, date: c.commit?.author?.date, url: c.html_url })) });
  } catch { res.json({ data: [] }); }
});

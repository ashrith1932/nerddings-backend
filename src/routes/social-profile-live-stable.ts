...
socialProfileLiveStableRouter.get("/users/:username/profile-live",async(req,res)=>{
  ...
  try{
    ...
    let affiliations:Row[]=[];try{
      const approved = await rows(sql`SELECT a.id,a.name,a.slug,a.type,a.website,a.verified,aa.role,aa.verified_at,true as is_verified,'accepted' as status FROM agent_affiliations aa JOIN agents a ON a.id=aa.agent_id WHERE aa.user_id=${user.id}`);
      let pending:Row[]=[];
      try {
        pending = await rows(sql`SELECT a.id,a.name,a.slug,a.type,a.website,a.verified,ar.role,ar.created_at as verified_at,false as is_verified,'pending' as status FROM agent_affiliation_requests ar JOIN agents a ON a.id=ar.agent_id WHERE ar.user_id=${user.id} AND ar.status='pending'`);
      } catch {}
      affiliations = [...approved, ...pending].sort((x,y)=>new Date(y.verified_at).getTime() - new Date(x.verified_at).getTime());
    }catch{}

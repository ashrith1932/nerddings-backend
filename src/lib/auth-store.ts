...
export type AuthUser = { id: string; name: string; username: string; email: string; accountType: "user" | "agent"; avatarUrl?: string | null; onboardingCompleted: boolean };
...
export async function registerUser(input: { name: string; username: string; email: string; password: string; accountType: "user" | "agent" }) {
  if (db) {
    ...
    return { id: created.id, name: created.name, username: created.username, email: created.email, accountType: (created.accountType === "agent" ? "agent" : "user") as "user" | "agent", avatarUrl: created.avatarUrl, onboardingCompleted: true };
  }
  ...
  const user: LocalUser = { id: randomUUID(), name: input.name, username: input.username.toLowerCase(), email: input.email.toLowerCase(), accountType: input.accountType, passwordHash: await hashPassword(input.password), onboardingCompleted: true };
...
export async function loginUser(email: string, password: string) {
  if (db) {
    ...
    return { id: user.id, name: user.name, username: user.username, email: user.email, accountType: (user.accountType === "agent" ? "agent" : "user") as "user" | "agent", avatarUrl: user.avatarUrl, onboardingCompleted: user.onboardingCompleted };
  }
...
export async function findOrCreateOAuthUser(input: { email: string; name?: string | null; username?: string | null; avatarUrl?: string | null }) {
  ...
  if (db) {
    ...
    if (existing) return { id: existing.id, name: existing.name, username: existing.username, email: existing.email, accountType: (existing.accountType === "agent" ? "agent" : "user") as "user" | "agent", avatarUrl: existing.avatarUrl, onboardingCompleted: existing.onboardingCompleted };
    ...
    const [created] = await db.insert(users).values({ name, username, email, avatarUrl: input.avatarUrl ?? null, onboardingCompleted: false }).returning();
    if (!created) throw new Error("OAUTH_USER_CREATE_FAILED");
    return { id: created.id, name: created.name, username: created.username, email: created.email, accountType: "user" as const, avatarUrl: created.avatarUrl, onboardingCompleted: false };
  }
  ...
  const user: LocalUser = { id: randomUUID(), name, username, email, accountType: "user", passwordHash: "", avatarUrl: input.avatarUrl ?? null, onboardingCompleted: false };
  localUsers.set(user.id, user);
  return publicUser(user);
}
...
export async function completeOnboarding(userId: string, input: { name: string; username: string; accountType: "user" | "agent"; interests: string[] }) {
  if (db) {
    ...
    const [updated] = await db.update(users).set({ name: input.name, username: input.username.toLowerCase(), accountType: input.accountType, interests: input.interests, onboardingCompleted: true }).where(eq(users.id, userId)).returning();
    if (!updated) throw new Error("ONBOARDING_FAILED");
    if (input.accountType === "agent") await db.insert(agents).values({ id: updated.id, name: updated.name, slug: updated.username, type: "startup", verified: false, verificationStatus: "pending" }).onConflictDoNothing();
    return { id: updated.id, name: updated.name, username: updated.username, email: updated.email, accountType: (updated.accountType === "agent" ? "agent" : "user") as "user" | "agent", avatarUrl: updated.avatarUrl, onboardingCompleted: true };
  }
  const user = localUsers.get(userId);
  if (!user) throw new Error("ONBOARDING_FAILED");
  user.name = input.name; user.username = input.username.toLowerCase(); user.accountType = input.accountType; user.onboardingCompleted = true;
  return publicUser(user);
}

...
/*
 * clientMessageId is intentionally ephemeral. It is only needed to
 * connect the instant optimistic message to the persisted DB message
 * while the two browsers are live. The DB timestamps remain the source
 * of truth after refresh/reconnect.
 */
...
function addClient(
  ...
  console.log(
    `[WS] User ${userId} connected (${sockets.size} socket${
      sockets.size === 1 ? "" : "s"
    })`,
  );
}
...
function removeClient(
  ...
  console.log(
    `[WS] User ${userId} disconnected (${sockets.size} socket${
      sockets.size === 1 ? "" : "s"
    })`,
  );
}
...
function sendToUser(
  ...
  if (
    ...
    console.log(
      `[WS] User ${userId} is offline; event not pushed`,
    );
    return 0;
  }
  ...
  console.log(
    `[WS] Pushed event to ${userId}: ${sent}/${sockets.size} socket(s)`,
  );
...
async function replayUndelivered(
  userId: string,
  socket: WebSocketLike,
) {
  if (!db) {
  ...
  if (count > 0) {
    console.log(
      `[WS] Replayed ${count} undelivered message(s) to ${userId}`,
    );
  }
}
        ...
        async (raw) => {
          try {
            ...
            if (
               ...
               * This prevents multiple browser tabs from moving it.
               */
              const deliveredAt =
                stored.deliveredAt ??
                new Date();
